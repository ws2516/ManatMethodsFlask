...
def gameToday():
	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49629.3.json').json()
	boolean = searchingForGame(jsonData_fanduel_epl)
	return boolean

def fetch():
  try:
  	jsonData_fanduel_nba = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49629.3.json').json() #gives the game id
  except:
...
def fetchName(): 
  ...
  jsonData_fanduel_nba = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/49629.3.json').json() #gives the game id
  url = 'https://projects.fivethirtyeight.com/2021-wnba-predictions/games/'
