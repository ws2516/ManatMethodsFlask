...
def gameToday():
	jsonData_fanduel_epl = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/60826.3.json').json()
	boolean = searchingForGame(jsonData_fanduel_epl)
	return boolean

def fetch():
  try:
  	jsonData_fanduel_mlb = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/60826.3.json').json() #gives the game id
  except:
  	print('Not a problem, the XHR has been changed for the NBA, go ahead and fix that then run again')
  mlb = parse_data(jsonData_fanduel_mlb)
  MLB = pd.DataFrame(mlb)[['eventname','tsstart','idfoevent.markets']]
  MLB.columns = ['Teams','Date','EventID']
  listing = []
  for i in np.unique(MLB.EventID.values): 
...
def fetchName(): 
  jsonData_fanduel_nba = requests.get('https://sportsbook.fanduel.com/cache/psmg/UK/60826.3.json').json() #gives the game id
  ...
  todays =  now.strftime("%A, %B %d").lstrip("0").replace(" 0", " ")
  masking = [i.text==todays for i in page_content.findAll("span", class_="day long")]
  trueMask = []
  for i in masking:
  	trueMask += [i]*2
  teams = [i.text for i in page_content.findAll("span", class_="team-name long")]
  teamsToday = []
  for j in range(len(teams)):
  	if trueMask[j]:
  		...
  		continue
  probabilitiesToday = [float(i.text[:-1])/100 for i in page_content.findAll('td', class_="td number td-number win-prob")[:len(teamsToday)]]
  indexed = []
  for i in range(int(len(teamsToday)/2)):
  	indexed += [i]*2
  MLB_Final = pd.DataFrame({'ID':teamsToday, 'Probabilities':probabilitiesToday, 'gameNum':indexed })
  return MLB_Final
